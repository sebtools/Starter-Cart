...
<cfcomponent displayname="Authorize.net AIM Interface" extends="cfpayment.api.gateway.base" hint="Authorize.net AIM Gateway" output="false">
	...
	<cfscript>
		...
		variables.cfpayment.GATEWAY_TEST_URL = "https://secure.authorize.net/gateway/transact.dll";
	...
	<cffunction name="process" output="false" access="private" returntype="any">
		...
		<cfscript>
			...
			structInsert(p, "x_password", getPassword(), "yes");
			...
			structInsert(p, "x_test_request", "FALSE", "yes"); // Maybe: getTestMode()
			...
			if (NOT response.hasError()) {
				...
				if (len(response.getResult())) {
					...
					if ( StructKeyExists(results, "x_trans_ID") AND results.x_trans_ID NEQ 0 ) {
						response.setTransactionID(results.x_trans_ID);
					} else {
						if ( StructKeyExists(p, "x_trans_ID") AND p.x_trans_ID NEQ 0 ) {
							response.setTransactionID(p.x_trans_ID);
						} else {
							//da(p=p,results=results,response_result=response.getResult());
						}
